...
<?= $this->section('styles') ?>
<style>
#blueprints-table tbody tr { cursor: pointer; transition: background var(--sap-transition); }
#blueprints-table tbody tr:hover { background: var(--sap-brand-hover); }
#blueprints-table_filter { display: none; }
...
$(function() {
    var table = $('#blueprints-table').DataTable({
        processing: true,
        serverSide: false,
        responsive: {
            details: {
        ...
        ajax: {
            url: site_url + '/blueprints/ajax-list',
            dataSrc: 'data'
        },
        columns: [
            {

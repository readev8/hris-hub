            ...
            <form id="blueprintForm">
                ...
                <div class="mb-4 pb-3" style="border-bottom:1px dashed var(--sap-border)">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-8">
                            ...
                            <textarea name="actors" class="sap-input" rows="2" placeholder="e.g., System Admin, Department Head, User"><?= esc($blueprint['actors'] ?? '') ?></textarea>
                        </div>
                        <div class="col-md-4">
                            <label class="sap-label">Frequency</label>
                            <select name="frequency" class="sap-select">
                                <option value="">-- Select --</option>
                                <option value="Daily" <?= ($blueprint['frequency'] ?? '') == 'Daily' ? 'selected' : '' ?>>Daily</option>
                                <option value="Weekly" <?= ($blueprint['frequency'] ?? '') == 'Weekly' ? 'selected' : '' ?>>Weekly</option>
                                <option value="Monthly" <?= ($blueprint['frequency'] ?? '') == 'Monthly' ? 'selected' : '' ?>>Monthly</option>
                                <option value="Quarterly" <?= ($blueprint['frequency'] ?? '') == 'Quarterly' ? 'selected' : '' ?>>Quarterly</option>
                                <option value="Yearly" <?= ($blueprint['frequency'] ?? '') == 'Yearly' ? 'selected' : '' ?>>Yearly</option>
                            </select>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="sap-label">Pre Condition</label>
                        <textarea name="pre_condition" class="sap-input" rows="3" placeholder="Conditions that must be true before this use case starts..."><?= esc($blueprint['pre_condition'] ?? '') ?></textarea>
                    </div>
                    <div class="mb-3">
                        <label class="sap-label">Post Condition</label>
                        <textarea name="post_condition" class="sap-input" rows="3" placeholder="State of the system after the use case completes..."><?= esc($blueprint['post_condition'] ?? '') ?></textarea>
                    </div>
                    <div class="mb-3">
                        <label class="sap-label">Normal Course</label>
                        <textarea name="normal_course" class="sap-input" rows="5" placeholder="Step-by-step description of the main flow..." style="min-height:100px"><?= esc($blueprint['normal_course'] ?? '') ?></textarea>
                    </div>
                    <div class="mb-3">
                        <label class="sap-label">Exception</label>
                        <textarea name="exception" class="sap-input" rows="3" placeholder="Error conditions and how they are handled..."><?= esc($blueprint['exception'] ?? '') ?></textarea>
                    </div>
                </div>
                ...
                <div class="mb-4 pb-3" style="border-bottom:1px dashed var(--sap-border)">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="notes" class="sap-input" rows="3" placeholder="Additional notes..."><?= esc($blueprint['notes'] ?? '') ?></textarea>
                    </div>
                    <div class="mb-3">
                        <label class="sap-label">Issue (Business Rules / Assumptions)</label>
                        <textarea name="issue" class="sap-input" rows="3" placeholder="Business rules, assumptions, and constraints..."><?= esc($blueprint['issue'] ?? '') ?></textarea>
                    </div>
                </div>

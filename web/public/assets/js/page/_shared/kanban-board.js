 ...
 * ============================================================================
 *
 * Reusable kanban board with Sortable.js drag-and-drop, transition rules,
 * and AJAX ticket movement. Used by master-projects detail and module_detail.
 *
 * Dependencies: jQuery, Sortable.js, Toastr, SweetAlert2
 * Date: 2026-08-18
 */

/* global $, site_url, toastr, Swal, Sortable, escHtml, GlobalSanitize */
...
const KanbanBoard = {
    ...
    API_ENDPOINTS: {
        MOVE_TICKET: function (ticketId) {
            return site_url + '/tickets/' + ticketId + '/move';
        }
    },
    ...
    render: function () {
        ...
        for (var c = 0; c < columns.length; c++) {
            ...
            if (!tickets.length) {
                $col.html(GlobalSanitize.sanitizeHtml('<div class="kanban-empty"><i class="fas fa-inbox"></i>No tickets</div>'));
                continue;
            }
            ...
            $col.html(GlobalSanitize.sanitizeHtml(html));
        }
    },
    ...
    _moveTicket: function (ticketId, newStatus, oldStatus, cardEl) {
        ...
        $.post(
            self.API_ENDPOINTS.MOVE_TICKET(ticketId),
            ...
            var res = null;
            try { res = JSON.parse(xhr.responseText); } catch (e) { /* parse error */ }
            if (res && res.redirect) {
...
$(document).on('dblclick', '.kanban-card', function () {
    ...
    Swal.fire({
        title: 'Buka Ticket?',
        html: GlobalSanitize.sanitizeHtml('<strong>' + escHtml(cardTitle) + '</strong>'),
        icon: 'question',

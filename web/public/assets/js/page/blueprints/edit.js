 ...
 * ============================================================================
 *
 * Description: Edit blueprint form with AJAX submission
 * Date: 2026-07-16
...
$(function() {
    ...
    $('#blueprintForm').on('submit', function(e) {
        ...
        $.ajax({
            url: site_url + '/blueprints/' + data.token + '/update',
            type: 'POST',
            data: $(this).serialize(),
            success: function(res) {
                if (res.status) {
                    toastr.success('Blueprint updated');
                    setTimeout(function() { window.location.href = site_url + '/blueprints/' + data.token; }, 500);
                } else {

...
$(function() {
    ...
    $('#myTicketsTable thead tr:last th').each(function(i) {
        ...
        $(this).html('<input type="text" class="column-search" placeholder="Search ' + $('#myTicketsTable thead tr:first th:eq(' + i + ')').text() + '..." data-col="' + i + '">');
    });

    $('#myTicketsTable').on('keyup change', '.column-search', function() {
...
function updateSummaryCards(data) {
    var counts = { 0: 0, 2: 0, 3: 0, 4: 0 };
    for (var i = 0; i < data.length; i++) {
        var s = data[i].status;
        if (s in counts) counts[s]++;
    }
    $('#countOpen').text(counts[0]);
    $('#countInProgress').text(counts[2]);
    $('#countResolved').text(counts[3]);
    $('#countClosed').text(counts[4]);
}

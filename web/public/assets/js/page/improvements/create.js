 ...
 * ============================================================================
 *
 * Create improvement form with file upload dropzone.
 *
...
const ImprovementCreate = {
    ...
    renderPreview: function () {
        var self = ImprovementCreate;
        var preview = $('#filePreview');
        preview.empty();
        for (var i = 0; i < self._selectedFiles.length; i++) {
            var file = self._selectedFiles[i];
            var idx = i;
    ...
    init: function () {
        ...
        $('#improvementForm input[name="images[]"]').on('change', function () {
            ...
            for (var i = 0; i < newFiles.length; i++) {
                ...
                var exists = self._selectedFiles.some(function (f) {
                    return f.name === newFiles[i].name && f.size === newFiles[i].size;
                });
                if (!exists) {
                    self._selectedFiles.push(newFiles[i]);
                }
            }
            if (self._selectedFiles.length > 5) {
                toastr.warning('Maximum 5 files');
                self._selectedFiles = self._selectedFiles.slice(0, 5);
            }
            self.renderPreview();
            $(this).val('');
        });
        ...
        $('#filePreview').on('click', '.btn-remove-file', function () {
            self._selectedFiles.splice($(this).data('idx'), 1);
            self.renderPreview();
        });
        ...
        $('#improvementForm').on('submit', function (e) {
            ...
            for (var i = 0; i < self._selectedFiles.length; i++) {
                formData.append('images[]', self._selectedFiles[i]);
            }
...
window.ImprovementCreate = ImprovementCreate;

 ...
 * ============================================================================
 *
 * Improvement detail with approval actions, comments, and attachments.
 *
...
const ImprovementDetail = {
    ...
    doAction: function (action) {
        $.post(site_url + '/improvements/' + ImprovementDetail._token + '/' + action, {}, function (res) {
            if (res.status) {
            ...
            toastr.error('Failed to perform action (HTTP ' + xhr.status + ')');
        });
    },

    promptReject: function () {
        Swal.fire({
            ...
            if (result.isConfirmed && result.value) {
                $.post(site_url + '/improvements/' + ImprovementDetail._token + '/reject', { notes: result.value }, function (res) {
                    if (res.status) {
                        toastr.success(res.data.message);
                        setTimeout(function () { location.reload(); }, 800);
                    } else {
                    ...
                    toastr.error('Failed to reject improvement (HTTP ' + xhr.status + ')');
                });
            }
        });
    },
    ...
    confirmDelete: function () {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post(site_url + '/improvements/' + ImprovementDetail._token + '/delete', {}, function (res) {
                    if (res.status) {
                    ...
                    toastr.error('Failed to delete improvement (HTTP ' + xhr.status + ')');
                });
            }
        });
    },
    ...
    init: function () {
        ...
        $('#commentForm').on('submit', function (e) {
            ...
            if (!text.trim()) return;
            $.post(site_url + '/improvements/' + self._token + '/comments', { content: text }, function (res) {
                if (res.status) {
        ...
        $('#attachmentInput').on('change', function () {
            ...
            $.ajax({
                url: site_url + '/improvements/' + self._token + '/attachments',
                type: 'POST',
                data: fd,
                processData: false,
                contentType: false,
...
window.ImprovementDetail = ImprovementDetail;

// Backward compatibility for onclick handlers in views
window.doAction = function (action) { ImprovementDetail.doAction(action); };
window.promptReject = function () { ImprovementDetail.promptReject(); };
window.confirmDelete = function () { ImprovementDetail.confirmDelete(); };
